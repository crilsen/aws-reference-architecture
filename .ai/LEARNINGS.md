...
### L-004 — EBS CSI add-on requires identity ordering
Date: 2026-09-14
Status: promoted
Confidence: observed
Scope: modules/eks/main.tf
Context: Applying the monolithic EKS Lab.
Evidence: AWS EKS add-on remained `CREATING`; source showed the Pod Identity association depended on the add-on.
Pattern / rule: Create EBS CSI Pod Identity before the EBS CSI add-on.
Promotion: DECISIONS.md#ADR-005

### L-005 — AL2023 runner bootstrap package pitfall
Date: 2026-09-14
Status: promoted
Confidence: observed
Scope: modules/github-runner/user-data.sh.tftpl
Context: Bootstrapping the private GitHub runner.
Evidence: EC2 console output reported `No match for argument: awscli2`, which halted cloud-init.
Pattern / rule: Do not install `awscli2` with dnf on this AL2023 image; verify the preinstalled AWS CLI instead.
Promotion: TOOLS.md
